...
def reverse_rows(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  # rev_idx = torch.LongTensor([i for i in range(x.shape[0]-1, -1, -1)])
  # y = x[rev_idx].clone()
  y = x[range(x.shape[0]-1, -1, -1)].clone()
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def make_one_hot(x):
  ...
  # Replace "pass" statement with your code
  x_size = len(x)
  y = torch.zeros(x_size, max(x) + 1, dtype=torch.float32)
  y[torch.arange(x_size), x] = 1
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def reshape_practice(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  y = torch.concatenate([x[:12].reshape(3, 4), x[12:].reshape(3, 4)], axis=1)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def zero_row_min(x):
  ...
  # Replace "pass" statement with your code
  y = x.clone()
  y[torch.arange(y.shape[0]), torch.min(x, axis=1).indices] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  # Replace "pass" statement with your code
  if use_loop:
      z = torch.stack([torch.matmul(x[i], y[i]) for i in range(x.shape[0])])
  else:
      z = torch.bmm(x, y)
  #############################################################################
  #                            END OF YOUR CODE                               #
